...
class FlightTripDatabaseTest : public ::testing::Test {
...
protected:
  ...
  void TearDown() {}
  FlightTripDatabase trips;
};

TEST_F(FlightTripDatabaseTest, AddTripTest) {
  trips.addTrip("Chennai","Delhi", "SG304","Spicejet", 1300);
  EXPECT_NE((Trip*)NULL, trips.findTripById("SG304"));
  EXPECT_EQ(7, trips.countAll());
}
TEST_F(FlightTripDatabaseTest, RemoveTripTest) {
  trips.removeTrip("6E501");
  EXPECT_EQ(NULL, trips.findTripById("6E501"));
  EXPECT_EQ(5, trips.countAll());
}
TEST_F(FlightTripDatabaseTest, CountTest) {             //Just After Setup
  EXPECT_EQ(6, trips.countAll());
}
TEST_F(FlightTripDatabaseTest, CountByOperatorTest) {  //Just After Setup
